//
//  OldTimetableNetworkController.swift
//  MyKTGG
//
//  Created by Алексей Трушковский on 19.03.2021.
//  Copyright © 2021 Алексей Трушковский. All rights reserved.
//

import UIKit

class OldTimetableNetworkController {
    ...
    func configureCell(cell: TimeTableCell, for indexPath: IndexPath, date: Date, isStudent: Bool, subGroup: Int) {
        ...
        guard date.ignoringTime == newDate?.ignoringTime else { return }
        guard var lesson = item[indexPath.row].lessonDescription else { return }
            if isStudent {
                if isTZ(lesson: lesson) {
                    ...
                    if isTZcancel(lesson: lesson) {
                        let lessonArr = lesson.components(separatedBy: "Увага! Заняття відмінено!")
